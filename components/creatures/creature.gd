...
func move_towards_follow_node(delta):
	if (body.linear_velocity.length() <= 800):
...
func rotate_to_follow_node(delta):
	...
	if (abs(difference) < deg_to_rad(10) ): 
		body.lock_rotation = true
	else:		
		body.lock_rotation = false
		
	if (difference > 0):
		body.apply_torque(10) 
	if (difference < 0):
		body.apply_torque(-10) 
				
	if (abs(difference) > PI/4):
		body.apply_central_force((body.global_position - node_to_follow.global_position).normalized() * delta * 10000) 	
